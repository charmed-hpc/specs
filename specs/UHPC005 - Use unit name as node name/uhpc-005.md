...
identify whether a component is provided by Juju or Slurm:

1. Compute node
   1. A machine that is managed by Juju using the slurmd charm and is 
      registered with the Slurm controller `slurmctld` for running user workloads.
2. Machine hostname
   1. The hostname of the machine managed by the slurmd charm.
3. Machine ID
   1. The unique ID number of the machine managed by Juju.
4. Node name
   1. The unique name that Slurm uses to refer to a compute node. 
5. Unit name
...
of the `"Machine"` section in the `juju status` output and match the unit's
associated machine ID number with the machine's hostname in the `"Inst id"` column.
